...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline) {
  ...
  if (finfo != 0 && finfo->size > sizeof(struct exec_header)) {
    ...
    int data_size = finfo->size +
                    256 * 1024; /* Default data size = file size + 256K stack */
    file_loadfile(finfo->clustno, sizeof(header), (char *)&header, fat,
                  (char *)(ADR_DISKIMG + 0x003e00)); /* Load header */
    if (strncmp(header.magic, "EXEC", 4) != 0) {
      cons_putstr0(cons, "EXEC format error.\n");
      return 1;
    }
    if (header.bss_len != 0) {
      data_size = header.bss_start + header.bss_len +
                  256 * 1024; /* alloc BSS section, bss start may > file size */
    }
    ...
    start_app(header.entry, 0 * 8 + 4, data_size - 1, 1 * 8 + 4,
              &(task->tss.esp0));
  ...
  if (edx == 1) {
    ...
    if ((ebx & 1) == 0) {
      sheet_refresh(sht, eax, ecx, esi + 1, edi + 1);
             ...
             11) { /* 8,9,10 is for malloc, we don't have such function */
    sht = (struct SHEET *)(ebx & 0xfffffffe);
    ...
    for (;;) {
      ...
      if (i == 4) {
        ...
        io_cli();
        fifo32_put(sys_fifo,
                   cons->sht - shtctl->sheets0 + 2024); /* 2024乣2279 */
        cons->sht = 0;
        io_sti();
      }
      if (i >= 256) {
...
void os_api_linewin(struct SHEET *sht, int x0, int y0, int x1, int y1,
                    int col) {
  int i, x, y, len, dx, dy;
